...
A take-home exercise implementing a tiny slice of a training platform — instructors publish **courses**, visitors submit **enrollments**,
...
**Dependency list** (internal module edges; each `-core` additionally pulls Spring Web/JPA):

| Module | Depends on |
|--------|-----------|
| `common` | — (JDK only) |
| `catalog-api` | `common` |
| `catalog-core` | `catalog-api`, `common` |
| `enrollments-api` | `common` |
| `enrollments-core` | `enrollments-api`, `catalog-api` *(sync seam)*, `common` |
| `notifications-api` | — (JDK only) |
| `notifications-core` | `notifications-api`, `enrollments-api` *(subscribes to its event)*, `common` |
| `app` | `catalog-core`, `enrollments-core`, `notifications-core` |
...
[`DomainBoundaryArchitectureTest`](app/src/test/java/com/test/app/architecture/DomainBoundaryArchitectureTest.java)
...
[`CoursesApi`](catalog-api/src/main/java/com/test/catalog/api/CoursesApi.java), which
returns a `CourseView` DTO. It never touches catalog's service, entity, or table. The single
implementation today ([`CoursesApiAdapter`](catalog-core/src/main/java/com/test/catalog/core/course/CoursesApiAdapter.java))
...
Producers publish through [`DomainEventPublisher`](common/src/main/java/com/test/common/event/DomainEventPublisher.java);
consumers implement [`DomainEventHandler`](common/src/main/java/com/test/common/event/DomainEventHandler.java).
Neither ever references Spring's `@EventListener` or a broker client. The **only** place the transport
is named is the app's [`DomainEventDispatcher`](app/src/main/java/com/test/app/event/DomainEventDispatcher.java)
+ [`SpringDomainEventPublisher`](app/src/main/java/com/test/app/event/SpringDomainEventPublisher.java).
...
- The app's `DomainEventDispatcher` already routes *any* `DomainEvent` to *any* `DomainEventHandler`
  by type, and component-scanning + `@EntityScan` over `com.test` auto-discover the new
...
- A second delivery channel (e.g. SMS) is added the same way — one new class implementing
  [`NotificationChannel`](notifications-api/src/main/java/com/test/notifications/api/NotificationChannel.java);
